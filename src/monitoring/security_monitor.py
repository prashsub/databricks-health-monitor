...
def main():
    ...
    try:
        ...
        if monitor:
            print("-" * 70)
            print("[✓ SUCCESS] Security access monitor created!")
            print(f"  Custom Metrics:  {num_metrics} configured")
            dbutils.notebook.exit(f"SUCCESS: Security monitor created with {num_metrics} metrics")
        else:
            print("-" * 70)
            print("[⊘ SKIPPED] Monitor already exists - no action needed")
            dbutils.notebook.exit("SKIPPED: Security monitor already exists")

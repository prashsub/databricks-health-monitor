...
def main():
    ...
    print(f"  Slicing:         workspace_id, cluster_id, node_type")
    ...
    try:
        ...
        if monitor:
            print("-" * 70)
            print("[✓ SUCCESS] Cluster utilization monitor created!")
            print(f"  Custom Metrics:  {num_metrics} configured")
            dbutils.notebook.exit(f"SUCCESS: Cluster monitor created with {num_metrics} metrics")
        else:
            print("-" * 70)
            print("[⊘ SKIPPED] Monitor already exists - no action needed")
            dbutils.notebook.exit("SKIPPED: Cluster monitor already exists")

...
CREATE TABLE IF NOT EXISTS {catalog}.{gold_schema}.alert_history (
  ...
  ai_analysis STRING
    COMMENT 'Short AI-generated analysis for TRIGGERED alerts. Business: provides immediate context without querying the agent. Technical: generated by FMAPI during batch evaluation, 2-3 sentences max.',
...
"""
    ...
    # Migrate existing alert_history tables: add ai_analysis column
    try:
        spark.sql(f"""
            ALTER TABLE {catalog}.{gold_schema}.alert_history
            ADD COLUMN ai_analysis STRING
            COMMENT 'Short AI-generated analysis for TRIGGERED alerts. Business: provides immediate context without querying the agent. Technical: generated by FMAPI during batch evaluation, 2-3 sentences max.'
        """)
        print("✓ Added ai_analysis column to alert_history")
    except Exception as e:
        if "already exists" in str(e).lower() or "FIELDS_ALREADY_EXISTS" in str(e):
            print("  ai_analysis column already exists (OK)")
        else:
            print(f"⚠ Could not add ai_analysis column: {e}")

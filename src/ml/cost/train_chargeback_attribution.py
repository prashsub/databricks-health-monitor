...
Attributes costs to appropriate cost centers.
...
def create_training_set_with_features(spark, fe, catalog, feature_schema, gold_schema):
    ...
    feature_names = ["daily_dbu", "daily_cost", "avg_dbu_7d", "avg_dbu_30d",
                     "jobs_on_all_purpose_cost", "serverless_cost", "dlt_cost",
                     "jobs_on_all_purpose_count", "cluster_count"]
    ...
    base_df = (spark.table(fact_usage)
               .select("workspace_id", F.to_date("usage_date").alias("usage_date"),
                       F.col("list_cost").alias("target_cost"))
               .groupBy("workspace_id", "usage_date")
               .agg(F.sum("target_cost").alias("target_cost")))
...
def log_model_with_feature_engineering(fe, model, training_set, metrics, hyperparams, catalog, feature_schema, X_train):
    ...
    with mlflow.start_run(run_name=get_run_name(model_name, "random_forest")) as run:
        mlflow.set_tags(get_standard_tags(
            model_name, "cost", "regression", "random_forest",
...
def main():
    import json
    print("\n" + "=" * 60)
    print("CHARGEBACK ATTRIBUTION - TRAINING")

...
def main():
    ...
    try:
        ...
        # Convert continuous failure_rate (0-1) to binary classification label
        # High failure rate (>0.2) = 1 (job likely to fail), else 0
        from pyspark.sql.functions import when, col
        training_df = training_df.withColumn(
            "high_failure_rate",
            when(col(LABEL_COLUMN) > 0.2, 1).otherwise(0)
        )
        
        X_train, X_test, y_train, y_test = prepare_training_data(
            training_df, available_features, "high_failure_rate", cast_label_to="int", stratify=True
        )
        ...
        result = log_model_with_features(
            ...
            MODEL_NAME, DOMAIN, "classification", ALGORITHM, "job_failure_prediction",

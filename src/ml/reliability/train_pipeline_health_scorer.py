...
Problem: Scoring/Regression
Algorithm: Random Forest Regressor
Domain: Reliability

Scores pipeline health based on historical patterns.
...
def create_training_set_with_features(spark, fe, catalog, feature_schema):
    ...
    # Feature names matching reliability_features table columns
    feature_names = ["total_runs", "avg_duration_sec", "success_rate", "failure_rate",
                     ...
                     "rolling_repair_rate_30d", "repair_runs"]  # Fixed: use actual column names
    ...
    # Health score: weighted combination of success rate and duration stability
    base_df = (spark.table(feature_table)
               .withColumn("health_score",
                           F.col("success_rate") * 0.5 +
                           (1 - F.least(F.col("duration_cv"), F.lit(1.0))) * 0.3 +
                           (1 - F.col("rolling_failure_rate_30d")) * 0.2)
               .select("job_id", "run_date", "health_score")
               .filter(F.col("health_score").isNotNull()))
...
def log_model_with_feature_engineering(fe, model, training_set, metrics, hyperparams, catalog, feature_schema, X_train):
    ...
    with mlflow.start_run(run_name=get_run_name(model_name, "random_forest")) as run:
        mlflow.set_tags(get_standard_tags(
            model_name, "reliability", "regression", "random_forest",
            "health_scoring", f"{catalog}.{feature_schema}.reliability_features"
        ))
...
def main():
    import json
    print("\n" + "=" * 60)
    print("PIPELINE HEALTH SCORER - TRAINING")
    ...
    setup_mlflow_experiment("pipeline_health_scorer")

...
Forecasts job execution duration for scheduling optimization.
...
def load_and_prepare_data(spark, catalog, feature_schema):
    ...
    # Actual columns: total_runs, avg_duration_sec, success_rate, duration_cv, rolling_avg_duration_30d, min/max_duration_sec
    feature_cols = ["total_runs", "avg_duration_sec", "success_rate", "duration_cv",
...
def train_and_log(X_train, X_test, y_train, y_test, feature_cols, catalog, feature_schema, spark):
    ...
    with mlflow.start_run(run_name=get_run_name(model_name, "gradient_boosting")) as run:
        mlflow.set_tags(get_standard_tags(model_name, "reliability", "regression", "gradient_boosting",
...
def main():
    print("\n" + "=" * 60 + "\nJOB DURATION FORECASTER - TRAINING\n" + "=" * 60)
    ...
    try:
        ...
        import json
        exit_summary = json.dumps({"status": "FAILED", "model": "duration_forecaster", "error": str(e)[:500]})

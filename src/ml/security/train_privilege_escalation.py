...
def create_training_set_with_features(spark, fe, catalog, feature_schema, X_train):
    ...
    # Feature names matching security_features table columns
    feature_names = ["event_count", "tables_accessed", "failed_auth_count", "unique_source_ips", 
                     "lateral_movement_risk", "is_activity_burst"]  # Fixed: use actual columns
...
def train_model(X, feature_names):
    ...
    hyperparams = {"n_estimators": 100, "contamination": 0.01, "random_state": 42}
...
def main():
    import json
    print("\n" + "=" * 60)
    print("PRIVILEGE ESCALATION DETECTOR - TRAINING")

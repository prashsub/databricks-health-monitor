...
def load_and_prepare_data(spark, catalog, feature_schema):
    ...
    feature_cols = ["event_count", "tables_accessed", "off_hours_events", "unique_source_ips", "sensitive_data_access"]
...
def train_and_log(X, feature_cols, catalog, feature_schema, spark):
    ...
    with mlflow.start_run(run_name=get_run_name(model_name, "isolation_forest")) as run:
        ...
        mlflow.log_params({"n_estimators": 100, "contamination": 0.02})
...
def main():
    print("\n" + "=" * 60 + "\nEXFILTRATION DETECTOR - TRAINING\n" + "=" * 60)

...
def log_model(fe, model, training_set, X_train, metrics, hyperparams, catalog, feature_schema, feature_table_full):
    ...
    with mlflow.start_run(run_name=get_run_name(MODEL_NAME, ALGORITHM)) as run:
        mlflow.set_tags(get_standard_tags(MODEL_NAME, DOMAIN, "anomaly_detection", ALGORITHM, "performance_regression_detection", feature_table_full))
        mlflow.log_params(hyperparams)
        mlflow.log_param("feature_table", feature_table_full)

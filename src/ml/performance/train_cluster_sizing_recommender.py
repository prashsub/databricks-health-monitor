...
"""
Train Cluster Sizing Recommender Model
======================================
...
Recommends optimal cluster sizing based on workload patterns.
"""
...
def load_training_data(spark, catalog, feature_schema):
    ...
    if label_col not in available_cols:
        ...
        if "avg_rows_produced" in available_cols:
            df = df.withColumn(label_col, 
                F.when(F.col("avg_rows_produced") > 1000000, 2)  # Large
                .when(F.col("avg_rows_produced") > 100000, 1)  # Medium
                .otherwise(0))  # Small
        else:
...
def log_model(model, X, metrics, hyperparams, catalog, feature_schema, feature_table, algorithm):
    ...
    with mlflow.start_run(run_name=get_run_name(model_name, algorithm)) as run:
        mlflow.set_tags(get_standard_tags(
            ...
            "cluster_sizing_recommendation", feature_table
        ))
...
def main():
    import json
    print("\n" + "=" * 60)
    print("CLUSTER SIZING RECOMMENDER - TRAINING")
    print("=" * 60)
    ...
    setup_mlflow_experiment("cluster_sizing_recommender")

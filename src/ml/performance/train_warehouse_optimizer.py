...
Optimizes SQL warehouse configurations.
...
def create_training_set_with_features(spark, fe, catalog, feature_schema):
    ...
    # Feature names matching performance_features table
    ...
    # Target: optimal cluster size recommendation (simplified)
    base_df = (spark.table(feature_table)
               .withColumn("optimal_size_score",
                           F.when(F.col("p95_duration_ms") > 30000, 3.0)  # Needs larger
                           .when(F.col("query_count") < 10, 1.0)  # Can be smaller
                           .otherwise(2.0))  # Optimal
               .select("warehouse_id", "query_date", "optimal_size_score"))
...
def prepare_and_train(training_df, feature_names):
    ...
    y = pdf["optimal_size_score"].fillna(2.0)
...
def main():
    import json
    print("\n" + "=" * 60)
    print("WAREHOUSE OPTIMIZER - TRAINING")

#!/usr/bin/env python3
"""
Validate that benchmark questions reference actual deployed assets - FIXED EXTRACTION.
"""
...
# ============================================================================
# Extract Ground Truth from Documentation - FIXED PATTERNS
# ============================================================================
...
def extract_tvf_names() -> Set[str]:
    ...
    # Pattern: | `get_xxx` | ... | in table
    tvf_pattern = r'\| `(get_[a-z_]+)` \|'
    tvfs = set(re.findall(tvf_pattern, content))
    
    print(f"✓ Found {len(tvfs)} TVFs: {sorted(list(tvfs))[:5]}...")
    return tvfs
...
def extract_metric_views() -> Set[str]:
    ...
    # Pattern: ### mv_xxx in markdown
    mv_pattern = r'### (mv_[a-z_]+)'
    metric_views = set(re.findall(mv_pattern, content))
    
    print(f"✓ Found {len(metric_views)} Metric Views: {sorted(list(metric_views))[:5]}...")
    return metric_views
...
def extract_monitor_tables() -> Set[str]:
    ...
    # Extract source tables from table
    source_pattern = r'\| `([a-z_]+)` \| [^|]+ \| [^|]+ \| \*\*Profile & Drift\*\*'
    ...
    print(f"✓ Found {len(monitor_tables)} Monitor Tables: {sorted(list(monitor_tables))[:5]}...")
...
def extract_ml_tables() -> Set[str]:
    ...
    # Pattern: | **xxx_predictions** | in table or ### xxx_predictions in markdown
    # Try both patterns
    ml_pattern1 = r'\| \*\*([a-z_]+(?:predictions|scores|recommendations))\*\* \|'
    ml_pattern2 = r'### ([a-z_]+(?:predictions|scores|recommendations))'
    
    ml_tables = set(re.findall(ml_pattern1, content))
    ml_tables.update(re.findall(ml_pattern2, content))
    
    print(f"✓ Found {len(ml_tables)} ML Tables: {sorted(list(ml_tables))[:5]}...")
    return ml_tables
...
def extract_gold_tables() -> Tuple[Set[str], Set[str]]:
    ...
    print(f"✓ Found {len(fact_tables)} Fact Tables: {sorted(list(fact_tables))[:5]}...")
    print(f"✓ Found {len(dim_tables)} Dim Tables: {sorted(list(dim_tables))[:5]}...")
...
def main():
    ...
    print("="*80)
    print("Benchmark Ground Truth Validation - FIXED EXTRACTION")
    print("="*80)

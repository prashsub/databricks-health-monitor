...
> **Monitoring Schema:** `dev_prashanth_subrahmanyam_system_gold_monitoring`

---

## Table of Contents

1. [ML Prediction Tables](#1-ml-prediction-tables)
2. [ML Feature Tables](#2-ml-feature-tables)
3. [Lakehouse Monitoring Tables](#3-lakehouse-monitoring-tables)
4. [Table-Valued Functions (TVFs)](#4-table-valued-functions-tvfs)
5. [Metric Views](#5-metric-views)

---

## 1. ML Prediction Tables

**Schema:** `dev_prashanth_subrahmanyam_system_gold_ml`

| Table Name | Type | Description |
|------------|------|-------------|
| `budget_forecast_predictions` | MANAGED | Budget forecasting predictions |
| `cache_hit_predictions` | MANAGED | Query cache hit rate predictions |
| `chargeback_predictions` | MANAGED | Cost chargeback predictions |
| `cluster_capacity_predictions` | MANAGED | Cluster capacity planning predictions |
| `commitment_recommendations` | MANAGED | Commitment purchase recommendations |
| `cost_anomaly_predictions` | MANAGED | Cost anomaly detection results |
| `data_drift_predictions` | MANAGED | Data drift detection results |
| `dbr_migration_predictions` | MANAGED | DBR version migration recommendations |
| `duration_predictions` | MANAGED | Job/query duration predictions |
| `exfiltration_predictions` | MANAGED | Data exfiltration risk predictions |
| `freshness_predictions` | MANAGED | Data freshness predictions |
| `job_cost_optimizer_predictions` | MANAGED | Job cost optimization recommendations |
| `job_failure_predictions` | MANAGED | Job failure probability predictions |
| `performance_regression_predictions` | MANAGED | Performance regression detection |
| `pipeline_health_predictions` | MANAGED | Pipeline health predictions |
| `privilege_escalation_predictions` | MANAGED | Privilege escalation risk detection |
| `query_optimization_predictions` | MANAGED | Query optimization recommendations |
| `query_performance_predictions` | MANAGED | Query performance predictions |
| `retry_success_predictions` | MANAGED | Job retry success probability |
| `security_threat_predictions` | MANAGED | Security threat detection results |
| `sla_breach_predictions` | MANAGED | SLA breach probability predictions |
| `tag_recommendations` | MANAGED | Resource tagging recommendations |
| `user_behavior_predictions` | MANAGED | User behavior anomaly detection |
| `warehouse_optimizer_predictions` | MANAGED | Warehouse optimization recommendations |
...
|------------|-------------|
| `cost_features` | Cost agent features for anomaly detection and forecasting. Captures daily DBU usage patterns, rolling statistics, and contextual features. |
| `performance_features` | Performance agent features for query and warehouse optimization. Captures query metrics and warehouse utilization patterns. |
| `quality_features` | Quality agent features for data governance monitoring. Captures catalog-level statistics and data quality metrics. |
| `reliability_features` | Reliability agent features for job failure and SLA prediction. Captures job execution patterns and historical trends. |
| `security_features` | Security agent features for threat and anomaly detection. Includes user type classification (HUMAN_USER, SERVICE_PRINCIPAL, SYSTEM), activity burst detection, and lateral movement risk indicators. |

---

## 3. Lakehouse Monitoring Tables

**Schema:** `dev_prashanth_subrahmanyam_system_gold_monitoring`

### Profile Metrics Tables

| Table Name | Source Fact Table | Description |
|------------|-------------------|-------------|
| `fact_usage_profile_metrics` | `fact_usage` | Billing/cost profiling metrics |
| `fact_job_run_timeline_profile_metrics` | `fact_job_run_timeline` | Job execution profiling metrics |
| `fact_query_history_profile_metrics` | `fact_query_history` | Query performance profiling metrics |
| `fact_audit_logs_profile_metrics` | `fact_audit_logs` | Security audit profiling metrics |
| `fact_node_timeline_profile_metrics` | `fact_node_timeline` | Cluster node profiling metrics |
| `fact_table_lineage_profile_metrics` | `fact_table_lineage` | Data lineage profiling metrics |

### Drift Metrics Tables

| Table Name | Source Fact Table | Description |
|------------|-------------------|-------------|
| `fact_usage_drift_metrics` | `fact_usage` | Cost/billing drift detection |
| `fact_job_run_timeline_drift_metrics` | `fact_job_run_timeline` | Job reliability drift detection |
| `fact_query_history_drift_metrics` | `fact_query_history` | Query performance drift detection |
| `fact_audit_logs_drift_metrics` | `fact_audit_logs` | Security event drift detection |
| `fact_node_timeline_drift_metrics` | `fact_node_timeline` | Cluster utilization drift detection |
| `fact_table_lineage_drift_metrics` | `fact_table_lineage` | Data lineage drift detection |

### Query Patterns

#### Profile Metrics Query Guide

```sql
-- Table-level KPIs
SELECT * FROM {table}_profile_metrics
...
SELECT * FROM {table}_profile_metrics
WHERE slice_key = '{dimension}';

-- Time filtering
SELECT * FROM {table}_profile_metrics
WHERE window.start >= '{start_date}' AND window.end <= '{end_date}';
```

#### Drift Metrics Query Guide

```sql
-- Period-over-period comparison
SELECT * FROM {table}_drift_metrics
WHERE drift_type = 'CONSECUTIVE' AND column_name = ':table';

-- Dimensional drift
SELECT * FROM {table}_drift_metrics
WHERE slice_key = '{dimension}' AND slice_value = '{value}';
```

### Available Metrics by Domain

| Domain | Profile Metrics | Drift Metrics | Slicing Dimensions |
|--------|-----------------|---------------|-------------------|
| **Billing** | `total_daily_cost`, `total_daily_dbu`, `tag_coverage_pct`, `serverless_ratio`, `jobs_cost_share`, `sql_cost_share` | `cost_drift_pct`, `dbu_drift_pct`, `tag_coverage_drift` | `workspace_id`, `sku_name`, `cloud`, `is_tagged`, `is_serverless` |
| **Reliability** | `total_runs`, `success_count`, `failure_count`, `success_rate`, `failure_rate`, `avg_duration_minutes`, `p95_duration_minutes`, `p99_duration_minutes` | `success_rate_drift`, `failure_count_drift`, `duration_drift_pct`, `p99_duration_drift_pct` | `workspace_id`, `result_state`, `trigger_type`, `job_name`, `termination_code` |
| **Query Performance** | `query_count`, `successful_queries`, `failed_queries`, `query_success_rate`, `avg_duration_sec`, `p95_duration_sec`, `p99_duration_sec`, `sla_breach_rate`, `cache_hit_rate`, `spill_rate` | `p95_duration_drift_pct`, `query_volume_drift_pct`, `failure_rate_drift`, `sla_breach_rate_drift` | `workspace_id`, `compute_warehouse_id`, `execution_status`, `statement_type`, `executed_by` |
| **Security** | `total_events`, `distinct_users`, `sensitive_action_count`, `failed_action_count`, `unauthorized_count`, `off_hours_rate`, `human_user_ratio`, `service_principal_ratio` | `event_volume_drift_pct`, `sensitive_action_drift`, `failure_rate_drift`, `off_hours_drift`, `user_count_drift` | `workspace_id`, `service_name`, `audit_level`, `action_name`, `user_identity_email` |
...
|---------------|-------------|----------------|
| `get_cost_mtd_summary()` | Month-to-date cost summary with MoM comparison | None |
| `get_cost_week_over_week(weeks_back)` | Weekly cost trends with growth analysis | `weeks_back` |
| `get_cost_trend_by_sku(start_date, end_date, sku_filter)` | Daily cost trend by SKU | `start_date`, `end_date`, `sku_filter` |
| `get_cost_anomalies(start_date, end_date, z_score_threshold)` | Statistical cost anomaly detection | `start_date`, `end_date`, `z_score_threshold` |
| `get_cost_by_owner(start_date, end_date, top_n)` | Cost attribution by owner | `start_date`, `end_date`, `top_n` |
| `get_cost_by_tag(start_date, end_date, tag_key)` | Cost breakdown by custom tag | `start_date`, `end_date`, `tag_key` |
| `get_top_cost_contributors(start_date, end_date, top_n)` | Top cost contributors by workspace/SKU | `start_date`, `end_date`, `top_n` |
| `get_cost_growth_analysis(entity_type, days_back, top_n)` | Cost growth analysis by entity | `entity_type`, `days_back`, `top_n` |
| `get_cost_growth_by_period(entity_type, top_n)` | Period-over-period cost growth | `entity_type`, `top_n` |
| `get_cost_forecast_summary(forecast_months)` | Cost forecasting based on historical trends | `forecast_months` |
| `get_commit_vs_actual(commit_year, annual_commit_amount, as_of_date)` | Commit tracking vs actual spend | `commit_year`, `annual_commit_amount`, `as_of_date` |
| `get_tag_coverage(start_date, end_date)` | Tag coverage metrics | `start_date`, `end_date` |
| `get_spend_by_custom_tags(start_date, end_date, tag_keys)` | Multi-tag cost analysis | `start_date`, `end_date`, `tag_keys` |
| `get_untagged_resources(start_date, end_date, resource_type)` | Untagged resource identification | `start_date`, `end_date`, `resource_type` |

### Job Reliability Functions

| Function Name | Description | Key Parameters |
|---------------|-------------|----------------|
| `get_job_success_rate(start_date, end_date, min_runs)` | Job success rate analysis | `start_date`, `end_date`, `min_runs` |
| `get_failed_jobs(start_date, end_date, workspace_filter)` | Failed job details | `start_date`, `end_date`, `workspace_filter` |
| `get_job_failure_trends(days_back)` | Daily failure trend analysis | `days_back` |
| `get_job_failure_costs(start_date, end_date, top_n)` | Cost of job failures | `start_date`, `end_date`, `top_n` |
| `get_job_repair_costs(start_date, end_date, top_n)` | Repair/retry cost analysis | `start_date`, `end_date`, `top_n` |
| `get_job_retry_analysis(start_date, end_date)` | Retry pattern analysis | `start_date`, `end_date` |
| `get_job_sla_compliance(start_date, end_date)` | SLA compliance by job | `start_date`, `end_date` |
| `get_job_duration_percentiles(days_back, job_name_filter)` | Duration percentile analysis | `days_back`, `job_name_filter` |
| `get_job_run_duration_analysis(days_back, min_runs, top_n)` | Duration statistics by job | `days_back`, `min_runs`, `top_n` |
| `get_job_run_details(job_id_filter, days_back)` | Detailed run history for a job | `job_id_filter`, `days_back` |
| `get_job_outlier_runs(days_back, min_baseline_runs, deviation_threshold)` | Outlier run detection | `days_back`, `min_baseline_runs`, `deviation_threshold` |
| `get_job_data_quality_status(days_back)` | Job-based data quality scoring | `days_back` |
| `get_most_expensive_jobs(start_date, end_date, top_n)` | Top expensive jobs | `start_date`, `end_date`, `top_n` |
| `get_job_spend_trend_analysis(days_back, top_n)` | Job spend trend analysis | `days_back`, `top_n` |
| `get_jobs_on_legacy_dbr(days_back)` | Jobs on legacy DBR versions | `days_back` |
| `get_jobs_without_autoscaling(days_back)` | Jobs without autoscaling | `days_back` |

### Query Performance Functions

| Function Name | Description | Key Parameters |
|---------------|-------------|----------------|
| `get_slow_queries(start_date, end_date, duration_threshold_seconds, top_n)` | Slow query identification | `start_date`, `end_date`, `duration_threshold_seconds`, `top_n` |
| `get_failed_queries(start_date, end_date, top_n)` | Failed query analysis | `start_date`, `end_date`, `top_n` |
| `get_high_spill_queries(start_date, end_date, min_spill_gb)` | High spill query detection | `start_date`, `end_date`, `min_spill_gb` |
| `get_query_latency_percentiles(start_date, end_date)` | Query latency percentiles by warehouse | `start_date`, `end_date` |
| `get_query_volume_trends(days_back, granularity)` | Query volume trends | `days_back`, `granularity` |
| `get_query_efficiency(start_date, end_date, min_duration_seconds, top_n)` | Query efficiency analysis | `start_date`, `end_date`, `min_duration_seconds`, `top_n` |
| `get_query_efficiency_analysis(hours_back, warehouse_filter)` | Warehouse efficiency metrics | `hours_back`, `warehouse_filter` |
| `get_warehouse_utilization(start_date, end_date)` | Warehouse utilization metrics | `start_date`, `end_date` |
| `get_user_query_summary(start_date, end_date, top_n)` | User query summary | `start_date`, `end_date`, `top_n` |

### Cluster/Compute Functions

| Function Name | Description | Key Parameters |
|---------------|-------------|----------------|
| `get_cluster_utilization(start_date, end_date, min_hours)` | Cluster utilization metrics | `start_date`, `end_date`, `min_hours` |
| `get_cluster_resource_metrics(days_back, top_n)` | CPU/memory resource metrics | `days_back`, `top_n` |
| `get_cluster_right_sizing_recommendations(days_back, min_observation_hours)` | Right-sizing recommendations | `days_back`, `min_observation_hours` |
| `get_underutilized_clusters(days_back, cpu_threshold, min_hours)` | Underutilized cluster detection | `days_back`, `cpu_threshold`, `min_hours` |
| `get_all_purpose_cluster_cost(start_date, end_date, top_n)` | All-purpose cluster cost analysis | `start_date`, `end_date`, `top_n` |

### Security Functions

| Function Name | Description | Key Parameters |
|---------------|-------------|----------------|
| `get_user_activity_summary(start_date, end_date, top_n)` | User activity summary with risk scoring | `start_date`, `end_date`, `top_n` |
| `get_user_activity_patterns(days_back, burst_threshold, exclude_system_accounts)` | User behavior pattern analysis | `days_back`, `burst_threshold`, `exclude_system_accounts` |
| `get_failed_actions(start_date, end_date, user_filter)` | Failed action details | `start_date`, `end_date`, `user_filter` |
| `get_security_events_timeline(start_date, end_date, user_filter)` | Security event timeline | `start_date`, `end_date`, `user_filter` |
| `get_sensitive_table_access(start_date, end_date, table_pattern)` | Sensitive table access audit | `start_date`, `end_date`, `table_pattern` |
| `get_permission_changes(start_date, end_date)` | Permission change tracking | `start_date`, `end_date` |
| `get_off_hours_activity(start_date, end_date, business_hours_start, business_hours_end)` | Off-hours activity detection | `start_date`, `end_date`, `business_hours_start`, `business_hours_end` |
| `get_ip_address_analysis(start_date, end_date)` | IP address analysis | `start_date`, `end_date` |
| `get_service_account_audit(days_back)` | Service account activity audit | `days_back` |

### Data Quality & Governance Functions

| Function Name | Description | Key Parameters |
|---------------|-------------|----------------|
| `get_table_freshness(freshness_threshold_hours)` | Table freshness monitoring | `freshness_threshold_hours` |
| `get_tables_failing_quality(freshness_threshold_hours)` | Tables failing quality checks | `freshness_threshold_hours` |
| `get_data_freshness_by_domain(freshness_threshold_hours)` | Freshness by domain | `freshness_threshold_hours` |
| `get_data_quality_summary()` | Overall data quality summary | None |
| `get_table_access_audit(start_date, end_date, table_pattern)` | Table access audit | `start_date`, `end_date`, `table_pattern` |
| `get_table_activity_status(days_back, inactive_threshold_days)` | Table activity status | `days_back`, `inactive_threshold_days` |
| `get_pipeline_data_lineage(days_back, entity_filter)` | Pipeline data lineage | `days_back`, `entity_filter` |
...
### Cost Analytics (`mv_cost_analytics`)

Comprehensive cost analytics for Databricks billing and usage analysis.

| Category | Dimensions |
|----------|------------|
| **Time** | `usage_date`, `usage_month`, `year`, `month`, `quarter`, `week_of_year`, `day_of_week` |
| **Entity** | `workspace_id`, `workspace_name`, `job_name`, `owned_by`, `run_as`, `entity_type` |
| **Product** | `sku_name`, `billing_origin_product`, `cloud`, `is_serverless` |
| **Tags** | `tag_status`, `team_tag`, `project_tag`, `cost_center_tag`, `env_tag` |

| Category | Measures |
|----------|----------|
| **Core Cost** | `total_cost`, `total_dbu`, `cost_per_dbu`, `usage_records`, `usage_days` |
| **Period Metrics** | `mtd_cost`, `ytd_cost`, `avg_daily_cost`, `last_7_day_cost`, `last_30_day_cost`, `prior_7_day_cost`, `prior_30_day_cost` |
| **Growth** | `week_over_week_growth_pct`, `month_over_month_growth_pct` |
| **Compute Type** | `serverless_cost`, `classic_compute_cost`, `serverless_ratio` |
| **Tagging** | `tagged_cost`, `untagged_cost`, `tag_coverage_pct` |
| **Activity** | `active_users`, `active_jobs`, `active_workspaces`, `unique_users_30d`, `unique_jobs_30d`, `unique_workspaces_30d` |

---

### Commit Tracking (`mv_commit_tracking`)

Contract commitment tracking and budget monitoring.

| Dimensions | Measures |
|------------|----------|
| `usage_date`, `usage_month`, `workspace_id`, `workspace_name`, `sku_name`, `billing_origin_product`, `cloud`, `is_serverless`, `owner`, `job_name` | `total_cost`, `total_dbu`, `mtd_cost`, `ytd_cost`, `daily_avg_cost`, `projected_monthly_cost`, `cost_per_dbu`, `serverless_cost`, `classic_compute_cost`, `serverless_ratio`, `usage_records`, `unique_jobs`, `unique_workspaces` |

---

### Job Performance (`mv_job_performance`)

Job execution performance metrics for reliability and efficiency analysis.

| Category | Dimensions |
|----------|------------|
| **Job** | `job_id`, `job_name`, `run_id`, `run_name`, `run_type` |
| **Time** | `run_date`, `year`, `month`, `week_of_year`, `day_of_week`, `hour_of_day` |
| **Status** | `result_state`, `termination_code`, `trigger_type`, `outcome_category`, `is_success` |
| **Workspace** | `workspace_id`, `workspace_name` |

| Category | Measures |
|----------|----------|
| **Counts** | `total_runs`, `successful_runs`, `failed_runs`, `timed_out_runs`, `cancelled_runs`, `runs_today`, `runs_last_7_days`, `failures_today`, `unique_jobs`, `unique_workspaces` |
| **Rates** | `success_rate`, `failure_rate`, `timeout_rate`, `success_rate_7d` |
| **Duration** | `avg_duration_minutes`, `p50_duration_minutes`, `p95_duration_minutes`, `p99_duration_minutes`, `max_duration_minutes`, `total_runtime_hours` |

---

### Query Performance (`mv_query_performance`)

SQL query execution performance metrics.

| Category | Dimensions |
|----------|------------|
| **Query** | `statement_id`, `statement_type`, `execution_status`, `is_from_cache`, `query_efficiency_status` |
| **Time** | `query_date`, `year`, `month`, `week_of_year`, `hour_of_day` |
| **Warehouse** | `warehouse_id`, `warehouse_name`, `warehouse_size`, `warehouse_tier` |
| **User** | `executed_by`, `executed_as`, `client_application` |
| **Workspace** | `workspace_name` |

| Category | Measures |
|----------|----------|
| **Counts** | `total_queries`, `successful_queries`, `failed_queries`, `cached_queries`, `queries_with_spill`, `inefficient_query_count`, `queries_today`, `queries_last_7_days`, `unique_users`, `unique_warehouses` |
| **Rates** | `cache_hit_rate`, `spill_rate`, `efficiency_rate` |
| **Duration** | `avg_duration_seconds`, `p50_duration_seconds`, `p95_duration_seconds`, `p99_duration_seconds`, `max_duration_seconds`, `total_duration_hours` |
| **Queue** | `avg_queue_time_seconds`, `p95_queue_time_seconds`, `p99_queue_time_seconds`, `avg_queue_to_execution_ratio`, `avg_compile_time_seconds` |
| **Data** | `avg_bytes_read_gb`, `avg_bytes_spilled_gb`, `total_bytes_read_tb`, `total_rows_read` |

---

### Security Events (`mv_security_events`)

Security and audit event analytics for compliance and access monitoring.

| Category | Dimensions |
|----------|------------|
| **Event** | `event_id`, `event_date`, `action_name`, `action_category`, `audit_level`, `is_sensitive_action`, `is_failed_action`, `response_status` |
| **Time** | `year`, `month`, `week_of_year`, `hour_of_day` |
| **User** | `user_email`, `user_type`, `is_human_user`, `source_ip` |
| **Service** | `service_name`, `workspace_id`, `workspace_name` |

| Category | Measures |
|----------|----------|
| **Counts** | `total_events`, `successful_events`, `failed_events`, `sensitive_events`, `authentication_events`, `permission_changes`, `delete_operations`, `events_24h`, `events_7d`, `events_prior_24h`, `failed_events_24h`, `sensitive_events_24h` |
| **Rates** | `failure_rate`, `sensitive_rate`, `event_growth_rate` |
| **Unique** | `unique_users`, `unique_human_users`, `unique_actions`, `unique_services`, `unique_ips` |

---

### Cluster Utilization (`mv_cluster_utilization`)

Cluster resource utilization and efficiency metrics.

| Category | Dimensions |
|----------|------------|
| **Cluster** | `cluster_id`, `cluster_name`, `cluster_source`, `node_type`, `is_driver` |
| **Time** | `utilization_date`, `year`, `month`, `week_of_year`, `hour_of_day` |
| **Status** | `provisioning_status`, `savings_opportunity` |
| **Workspace** | `workspace_id`, `workspace_name` |

| Category | Measures |
|----------|----------|
| **CPU** | `avg_cpu_utilization`, `peak_cpu_utilization`, `avg_cpu_user`, `avg_cpu_system`, `avg_cpu_wait` |
| **Memory** | `avg_memory_utilization`, `peak_memory_utilization`, `avg_swap_usage` |
| **Capacity** | `measurement_count`, `total_node_hours`, `wasted_hours`, `underprovisioned_hours`, `unique_clusters`, `unique_nodes` |
| **Efficiency** | `resource_efficiency_score`, `wasted_capacity_pct`, `potential_savings_pct` |
| **Samples** | `overprovisioned_samples`, `underprovisioned_samples`, `optimal_samples` |
| **Network** | `total_network_received_gb`, `total_network_sent_gb` |

---

### Cluster Efficiency (`mv_cluster_efficiency`)

Cluster efficiency and right-sizing analysis.

| Category | Dimensions |
|----------|------------|
| **Cluster** | `cluster_id`, `cluster_name`, `cluster_type`, `node_type`, `node_role`, `is_driver` |
| **Time** | `date`, `year`, `month`, `week_of_year` |
| **Status** | `cpu_utilization_tier`, `memory_utilization_tier`, `is_underutilized`, `is_overprovisioned` |
| **Workspace** | `workspace_id`, `workspace_name` |

| Category | Measures |
|----------|----------|
| **CPU** | `avg_cpu_utilization`, `max_cpu_utilization`, `avg_cpu_user_percent`, `avg_cpu_system_percent`, `avg_cpu_wait_percent` |
| **Memory** | `avg_memory_utilization`, `max_memory_utilization` |
| **Efficiency** | `efficiency_score`, `idle_percentage`, `underutilized_percentage` |
| **Node Counts** | `total_nodes`, `driver_nodes`, `worker_nodes`, `unique_clusters`, `underutilized_node_count`, `overprovisioned_node_count` |
| **Hours** | `total_node_hours`, `idle_hours`, `idle_node_hours_total`, `low_utilization_node_hours` |

---

### Data Quality (`mv_data_quality`)

Data freshness and quality monitoring for Gold layer tables.

| Category | Dimensions |
|----------|------------|
| **Table** | `table_catalog`, `table_schema`, `table_name`, `table_full_name`, `table_type`, `table_category` |
| **Classification** | `domain`, `is_fact_table`, `is_dim_table`, `freshness_status`, `is_stale` |
| **Time** | `last_modified`, `last_update_date`, `created_date` |

| Category | Measures |
|----------|----------|
| **Freshness** | `hours_since_update`, `avg_hours_since_update`, `min_hours_since_update`, `max_hours_since_update` |
| **Counts** | `total_tables`, `fresh_tables`, `stale_tables`, `critical_stale_tables`, `fact_tables`, `dim_tables`, `unique_schemas`, `unique_domains` |
| **Rates** | `freshness_rate`, `staleness_rate` |

---

### Governance Analytics (`mv_governance_analytics`)

Data lineage and governance tracking.

| Category | Dimensions |
|----------|------------|
| **Entity** | `entity_type`, `entity_id`, `created_by`, `operation_type` |
| **Source** | `source_table_full_name`, `source_table_name`, `source_schema`, `source_catalog`, `source_type` |
| **Target** | `target_table_full_name`, `target_table_name`, `target_schema`, `target_catalog`, `target_type` |
| **Time** | `event_date`, `year`, `month`, `week_of_year` |
| **Status** | `activity_status` |
| **Workspace** | `workspace_id`, `workspace_name` |

| Category | Measures |
|----------|----------|
| **Events** | `total_events`, `read_events`, `write_events`, `events_today`, `events_7d`, `events_30d`, `notebook_events`, `dashboard_events`, `job_events`, `pipeline_events` |
| **Tables** | `unique_tables`, `unique_source_tables`, `unique_target_tables`, `active_table_count`, `inactive_table_count` |
| **Users/Entities** | `unique_users`, `unique_notebooks`, `unique_jobs`, `unique_pipelines` |
| **Ratios** | `read_write_ratio` |

---

### ML Intelligence (`mv_ml_intelligence`)

ML model predictions and anomaly detection results.

| Category | Dimensions |
|----------|------------|
| **Prediction** | `prediction_date`, `model_version`, `risk_level`, `is_anomaly` |
| **Time** | `year`, `month`, `quarter`, `week_of_year`, `day_of_week` |
| **Entity** | `workspace_id`, `workspace_name`, `sku_name` |

| Category | Measures |
|----------|----------|
| **Predictions** | `total_predictions`, `anomaly_count`, `prediction_days` |
| **Risk** | `high_risk_count`, `critical_count` |
| **Scores** | `avg_anomaly_score`, `max_anomaly_score`, `anomaly_rate` |
| **Cost Impact** | `anomaly_cost`, `normal_cost`, `anomaly_cost_pct` |
| **Unique** | `unique_workspaces`, `unique_skus` |
...
| Asset Type | Count |
|------------|-------|
| **ML Prediction Tables** | 24 |
| **ML Feature Tables** | 5 |
| **Lakehouse Monitoring Tables** | 12 |
| **Table-Valued Functions** | 54 |
| **Metric Views** | 10 |
| **Total Assets** | 105 |
